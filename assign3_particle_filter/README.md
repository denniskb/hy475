...
Next we want to calculate our particles' weights or fitness. It is a number which expresses how well the particle models the plane's location. A high value means "very much" while a low value means "not at all". Don't worry about the exact numerical value, the weights will be normalized later anyway. What's important is their relative value to one another.
